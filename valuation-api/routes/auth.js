...
function passwordResetAcceptedPayload(extra = {}) {
  return {
    ok: true,
    message: "If that account exists, a password reset link will be sent shortly.",
    ...extra,
  };
}

function issueSession(res, user, license) {
  ...
  return sanitizeUser(user, license);
}
...
router.post("/signup", authLimiter, async (req, res) => {
  ...
  const license = createLicenseForUser(user.id, createLicenseKey());
  const sessionUser = issueSession(res, user, license);
...
router.post("/login", authLimiter, async (req, res) => {
  ...
  const license = findPrimaryLicenseForUser(user.id);
  const sessionUser = issueSession(res, user, license);
  return res.json({
    authenticated: true,
    user: sessionUser,
...
router.post("/password-reset/request", passwordResetLimiter, async (req, res) => {
  ...
  return res.status(202).json(
    passwordResetAcceptedPayload(exposeToken ? { resetToken: rawToken, resetUrl } : {}),
  );
});
...
router.post("/password-reset/confirm", passwordResetLimiter, async (req, res) => {
  ...
  updateUserPassword(record.user_id, await hashPassword(password));
