...
function makePlayerRecord(raw, overallIndex, tierRanks) {
  ...
  if (raw.roleBucket === "pitcher") {
    return {
      ...
      depth: tier,
      fpts: raw.fpts,
      projected_games: Math.round(safeNumber(raw.pitching.gamesPlayed)),
      projected_innings: Number((safeNumber(raw.pitching.outsPitched) / 3).toFixed(1)),
      mlbId: raw.mlbId,
  ...
  return {
    ...
    depth: tier,
    fpts: raw.fpts,
    projected_games: Math.round(safeNumber(raw.hitting.gamesPlayed)),
    projected_plate_appearances: Math.round(safeNumber(raw.hitting.plateAppearances)),
    projected_at_bats: Math.round(safeNumber(raw.hitting.atBats)),
    mlbId: raw.mlbId,
    overall_rank: overallIndex + 1,
    tier_rank: tierRanks[tier],
    age: raw.age,
